...
const Iphone = () => {
  ...
  useEffect(() => {
    ...
    setAvailableNFTs(
      initialAvailableNFTs.filter((_, index) => index !== randomIndex)
    );
  ...
  const handleSwipeEnd = () => {
    if (!swipeStart) return; // Guard clause to prevent processing if no swipe started
    ...
    if (
      Math.abs(swipeOffset) > swipeThreshold ||
      absVelocity > velocityThreshold
    ) {
      if (
        swipeOffset > 0 ||
        (absVelocity > velocityThreshold && velocityRef.current > 0)
      ) {
        handleLike();
      } else {
        handleDislike();
  ...
  const showNextNFT = () => {
    ...
    setTimeout(() => {
      ...
      // Completely reset all swipe-related state
      velocityRef.current = 0;
      lastPositionRef.current = 0;
      lastTimeRef.current = 0;
  ...
  const handleLike = () => {
    ...
    console.log("Liked:", currentNFT.title || currentNFT.name);
    showNextNFT();
  };
  ...
  const handleDislike = () => {
    setSwipeOffset(-300);
    // Update to use title instead of name
    console.log("Disliked:", currentNFT.title || currentNFT.name);
    showNextNFT();
  };
